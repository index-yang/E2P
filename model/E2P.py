...
class Cross_Attention(nn.Module):
    def __init__(self, 
        ...
        sqrt_q_dim = int(math.sqrt(625))
        self.resize = transforms.Resize([sqrt_q_dim, sqrt_q_dim])

        # self.resize = transforms.Resize([32, 16])
        self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1))

        self.q = nn.Linear(q_dim, 625, bias=bias)
...
def measure_inference_time(model, input_tensor, query_tensor, num_runs=100, warmup_runs=10):
    model.eval()
    ...
    with torch.no_grad():
        for _ in range(warmup_runs):
            _ = model(input_tensor, query_tensor)
            if torch.cuda.is_available():
                torch.cuda.synchronize()
...
# 	net = E2P().to("cuda" if torch.cuda.is_available() else "cpu")
# 	input = torch.randn(1, 3, 224, 224).to("cuda" if torch.cuda.is_available() else "cpu")
# 	query = torch.Tensor(np.random.random((1, 512))).to("cuda" if torch.cuda.is_available() else "cpu")
# 	avg_time = measure_inference_time(net, input, query, num_runs=100, warmup_runs=10)
